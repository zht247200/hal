...
bool hdl_parser_verilog::parse_assign(const std::string &token, const int line)
{
    ...
    {
        log_debug("hdl_parser", "Aliasing net {} to {}.", nets_rhs[i], nets_lhs[i]);
        auto rhs_net = m_net[nets_rhs[i]];
        if (rhs_net->get_data_by_key("alias", "idx") == std::make_tuple("", ""))
        {
            rhs_net->set_data("alias", "idx", "string", "0");
        }
        auto idx = std::stoi(std::get<1>(rhs_net->get_data_by_key("alias", "idx")));
        idx++;
        rhs_net->set_data("alias", std::to_string(idx), "string", nets_lhs[i]);
        rhs_net->set_data("alias", "idx", "string", std::to_string(idx));
    }
    return true;
}
