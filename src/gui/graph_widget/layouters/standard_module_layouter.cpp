#include "graph_widget/layouters/standard_module_layouter.h"
...
standard_module_layouter::standard_module_layouter(const module_context* const context) : module_layouter(context)
{
...
void standard_module_layouter::layout()
{
    // CLEANUP
...
void standard_module_layouter::create_boxes()
{
    // ZERO GATES
    int level_x = 0;
    ...
    // ARTIFICIAL SCOPE TO AVOID SHADOWING
    {
        ...
        int root = static_cast<int>(sqrt(m_zero_nodes.size()));
    ...
    for (QVector<hal::node>& v : m_positive_nodes)
    {
        ...
        int root = static_cast<int>(sqrt(v.size()));
...
void standard_module_layouter::calculate_nets()
{
    for (const u32 id : m_context->internal_nets()) // CURRENTLY IGNORES OTHER NETS
    {
...
void standard_module_layouter::find_max_box_dimensions()
{
...
void standard_module_layouter::find_max_channel_lanes()
{
...
void standard_module_layouter::calculate_max_channel_dimensions()
{
...
void standard_module_layouter::calculate_gate_offsets()
{
...
void standard_module_layouter::place_gates()
{
    for (node_box& box : m_boxes)
    {
...
void standard_module_layouter::update_scene_rect()
{
...
void standard_module_layouter::reset_roads_and_junctions()
{
...
void standard_module_layouter::draw_nets()
{
...
void standard_module_layouter::clear_net_layout_data()
{
    ...
    for (const standard_module_layouter::road* r : m_v_roads)
        delete r;
    m_v_roads.clear();

    for (const standard_module_layouter::junction* j : m_junctions)
        delete j;
    m_junctions.clear();
}

standard_module_layouter::node_box standard_module_layouter::create_box(const hal::node& node, const int x, const int y) const
{
    node_box box;
    box.node = node;
...
{
    for (const standard_module_layouter::node_box& box : m_boxes)
        if (box.x == x && box.y == y)
            return true;
...
bool standard_module_layouter::h_road_jump_possible(const int x, const int y1, const int y2) const
{
    if (y1 == y2)
        return false;
...
bool standard_module_layouter::h_road_jump_possible(const standard_module_layouter::road* const r1, const standard_module_layouter::road* const r2) const
{
    // CONVENIENCE METHOD
    assert(r1 && r2);
...
bool standard_module_layouter::v_road_jump_possible(const int x1, const int x2, const int y) const
{
    if (x1 == x2)
        return false;
...
bool standard_module_layouter::v_road_jump_possible(const standard_module_layouter::road* const r1, const standard_module_layouter::road* const r2) const
{
    // CONVENIENCE METHOD
    assert(r1 && r2);
...
standard_module_layouter::road* standard_module_layouter::get_h_road(const int x, const int y)
{
    for (standard_module_layouter::road* r : m_h_roads)
        if (r->x == x && r->y == y)
            return r;

    standard_module_layouter::road* new_r = new road(x, y);
    m_h_roads.append(new_r);
    return new_r;
}

standard_module_layouter::road* standard_module_layouter::get_v_road(const int x, const int y)
{
    for (standard_module_layouter::road* r : m_v_roads)
        if (r->x == x && r->y == y)
            return r;
...
standard_module_layouter::junction* standard_module_layouter::get_junction(const int x, const int y)
{
    for (standard_module_layouter::junction* j : m_junctions)
        if (j->x == x && j->y == y)
            return j;

    standard_module_layouter::junction* new_j = new junction(x, y);
    m_junctions.append(new_j);
    return new_j;
}

qreal standard_module_layouter::h_road_height(const unsigned int lanes) const
{
...
qreal standard_module_layouter::v_road_width(const unsigned int lanes) const
{
    // LANES COUNTED FROM 1
...
qreal standard_module_layouter::scene_y_for_h_channel_lane(const int y, const unsigned int lane) const
{
...
qreal standard_module_layouter::scene_x_for_v_channel_lane(const int x, const unsigned int lane) const
{
    // LINES NUMBERED FROM 0
...
qreal standard_module_layouter::scene_x_for_close_left_lane_change(const int channel_x, unsigned int lane_change) const
{
...
qreal standard_module_layouter::scene_x_for_far_left_lane_change(const int channel_x, unsigned int lane_change) const
{
    // LANE CHANGES COUNTED FROM 0
    assert(m_node_offset_for_x.contains(channel_x) || m_node_offset_for_x.contains(channel_x - 1));
...
qreal standard_module_layouter::scene_x_for_close_right_lane_change(const int channel_x, unsigned int lane_change) const
{
    // LANE CHANGES COUNTED FROM 0
    assert(m_node_offset_for_x.contains(channel_x) || m_node_offset_for_x.contains(channel_x - 1));
...
qreal standard_module_layouter::scene_x_for_far_right_lane_change(const int channel_x, unsigned int lane_change) const
{
    // LANE CHANGES COUNTED FROM 0
    assert(m_node_offset_for_x.contains(channel_x) || m_node_offset_for_x.contains(channel_x - 1));
...
qreal standard_module_layouter::scene_y_for_close_top_lane_change(const int channel_y, unsigned int lane_change) const
{
    // LANE CHANGES COUNTED FROM 0
    if (channel_y == 0)
...
qreal standard_module_layouter::scene_y_for_far_top_lane_change(const int channel_y, unsigned int lane_change) const
{
    // LANE CHANGES COUNTED FROM 0
    if (channel_y == 0)
        return m_node_offset_for_y.value(channel_y) -
...
qreal standard_module_layouter::scene_y_for_close_bottom_lane_change(const int channel_y, unsigned int lane_change) const
{
    // LANE CHANGES COUNTED FROM 0
    if (channel_y == 0)
        return m_node_offset_for_y.value(channel_y) -
...
qreal standard_module_layouter::scene_y_for_far_bottom_lane_change(const int channel_y, unsigned int lane_change) const
{
    // LANE CHANGES COUNTED FROM 0
    if (channel_y == 0)
        return m_node_offset_for_y.value(channel_y) -
...
qreal standard_module_layouter::scene_x_for_close_left_lane_change(const junction* const j) const
{
    // CONVENIENCE METHOD
    assert(j);
...
qreal standard_module_layouter::scene_x_for_far_left_lane_change(const standard_module_layouter::junction* const j) const
{
    // CONVENIENCE METHOD
    assert(j);
...
qreal standard_module_layouter::scene_x_for_close_right_lane_change(const junction* const j) const
{
    // CONVENIENCE METHOD
    assert(j);
...
qreal standard_module_layouter::scene_x_for_far_right_lane_change(const standard_module_layouter::junction* const j) const
{
    // CONVENIENCE METHOD
    assert(j);
...
qreal standard_module_layouter::scene_y_for_close_top_lane_change(const junction* const j) const
{
    // CONVENIENCE METHOD
    assert(j);
...
qreal standard_module_layouter::scene_y_for_far_top_lane_change(const standard_module_layouter::junction* const j) const
{
    // CONVENIENCE METHOD
    assert(j);
...
qreal standard_module_layouter::scene_y_for_close_bottom_lane_change(const junction* const j) const
{
    // CONVENIENCE METHOD
    assert(j);
...
qreal standard_module_layouter::scene_y_for_far_bottom_lane_change(const standard_module_layouter::junction* const j) const
{
    // CONVENIENCE METHOD
    assert(j);
...
void standard_module_layouter::store_max(QMap<T1, T2>& map, T1 key, T2 value)
{
    if (map.contains(key))
...
void standard_module_layouter::commit_used_paths(const standard_module_layouter::used_paths& used)
{
...
void standard_module_layouter::recalculate_levels()
{
    ...
    std::shared_ptr<module> m = g_netlist->get_module_by_id(m_context->get_id());

    // LEVEL ZERO
    QVector<u32> level_zero_gates;

    for (const std::shared_ptr<net>& n : m->get_input_nets())
    {
        for (const endpoint& e: n->get_dsts())
            level_zero_gates.append(e.get_gate()->get_id());
    }
